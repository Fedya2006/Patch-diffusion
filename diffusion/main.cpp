...
public:
    Patch_based_AD(int r, int n_iters, int func, float k, float kr, float ks, float lambda, float sigma)
...
private:
    float l(const Mat& img)
    {
        Mat img_N = img(Rect( r + 1, r + 1 - 1, img.cols - 2 * (r + 1), img.rows - 2 * (r + 1))).clone();
        Mat img_S = img(Rect(r + 1, r + 1 + 1, img.cols - 2 * (r + 1), img.rows - 2 * (r + 1))).clone();
        Mat img_W = img(Rect(r + 1 - 1, r + 1, img.cols - 2 * (r + 1), img.rows - 2 * (r + 1))).clone();
        Mat img_E = img(Rect(r + 1 + 1, r + 1, img.cols - 2 * (r + 1), img.rows - 2 * (r + 1))).clone();
        ...
        cv::sqrt(sum3, sum3);
        float sum = 0;
        for(int a = 0; a < sum3.rows; a++)
            for(int b = 0; b < sum3.cols; b++)
                sum += sum3.at<float>(a,b);
        //double sum = cv::sum(sum3).val[0];
        sum = sum / 2;
        return lambda * (1.0f - float(exp(-float((pow(sum, 2))) / float(pow(float(img.rows - 2 * (r + 1))
                                                                            * float(img.cols - 2 * (r + 1)) * sigma, 2)))));
    }
    /*float l(const Mat& img) const
    {
        ...
        double sum = cv::sum(sum3).val[0];
        sum = sum / 2;
        return lambda * (1.0 - double(exp(-double((pow(sum, 2))) / double(pow((img.rows - 2 * (r + 1))
                                                                              * (img.cols - 2 * (r + 1)) * sigma, 2)))));
    }*/
        ...
        for (int ip = -r; ip <= r; ip++)
            for (int jp = -r; jp <= r; jp++)
            {
                ...
                for (int iq = -r; iq <= r; iq++)
                    for (int jq = -r; jq <= r; jq++)
                    {
                        __m256 w = _mm256_sub_ps(_mm256_loadu_ps(q + iq * stride + jq), v); // w = img - v
...
public:
    Mat anisotropic_diffusion(const Mat& img)
    {
        Mat Ipred(img.rows + 2 * (r + 1), img.cols + 2 * (r + 1), CV_32FC1);

        copyMakeBorder(img, Ipred, r + 1, r + 1, r + 1, r + 1, BORDER_REFLECT);
        ...
        for (int n = 0; n < n_iters; n++)
        {
            ...
            tbb::parallel_for(r + 1, Ipred.rows - r - 1, [&](int i)
            {
                _mm256_zeroupper();

                int j1 = Ipred.cols - 2 * r - 2;
                int j8 = j1 / 8 * 8;
        ...
        Mat final = Ipred(Rect(r + 1, r + 1, Ipred.rows - 2 * (r + 1), Ipred.cols - 2 * (r + 1) ));
        return final;
    }
};
...
public:
    Mat anisotropic_diffusion(const Mat& img)
    {
        Mat Ipred(img.rows + 2, img.cols + 2, CV_32FC1);
        copyMakeBorder(img, Ipred, 1, 1, 1, 1, BORDER_REFLECT);
        Mat Inext = Ipred.clone();
        for (int n = 0; n < n_iters; n++)
        {
            tbb::parallel_for(1, Ipred.rows - 1, [&](int i)
            {
                for (int j = 1; j < Ipred.cols - 1; j++)
                {
                    float sum = 0;
        ...
        Mat final = Ipred(Rect(1, 1, Ipred.rows - 2, Ipred.cols - 2));
        return final;
    }
};
